...
bool ContactMap::processDcdData(const Config& config, const Int& psf_natom) {
    ...
    if(!inp_file.is_open()) return error_exit<bool>("Cannot open DCD file.", false);
    // Process DCD header
    DCD_Info dcd_info;
    char dcd_head1[100];
    char dcd_head2[16];
    ...
    if(strncmp(&dcd_head1[4], "CORD", 4)!=0) return error_exit<bool>("Wrong DCD format", false);
	...
	std::cout << "ReadDCD> (" << n_atom 
              << ") atoms found in trajectory file." << std::endl;
    if(n_atom != psf_natom) 
